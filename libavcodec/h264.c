...
static av_always_inline void hl_decode_mb_internal(H264Context *h, int simple){
    ...
    const int transform_bypass = !simple && (s->qp == 0 && h->sps.transform_bypass);
            ...
            if(IS_INTRA4x4(mb_type)){
                            ...
                            if(transform_bypass && h->sps.profile_idc==244 && dir<=1){
                                ...
                                if(nnz){
                                    if(is_h264){
                                        ...
                                        ff_svq3_add_idct_c(ptr, h->mb + i*16, linesize, s->qp, 0);
                                }
                            }
                        }
                    }
                ...
                if(is_h264){
                    if(h->non_zero_count_cache[ scan8[LUMA_DC_BLOCK_INDEX] ]){
                        if(!transform_bypass)
                            h->h264dsp.h264_luma_dc_dequant_idct(h->mb, h->mb_luma_dc, h->dequant4_coeff[0][s->qp][0]);
                        else{
                    ...
                    ff_svq3_luma_dc_dequant_idct_c(h->mb, h->mb_luma_dc, s->qp);
            }
            if(h->deblocking_filter)
            ...
            if(is_h264){
                ...
                for(i=0; i<16; i++){
                    if(h->non_zero_count_cache[ scan8[i] ] || h->mb[i*16]){ //FIXME benchmark weird rule, & below
                        uint8_t * const ptr= dest_y + block_offset[i];
                        ff_svq3_add_idct_c(ptr, h->mb + i*16, linesize, s->qp, IS_INTRA(mb_type) ? 1 : 0);
                    }
                }
            }
        }
                ...
                if(is_h264){
                    ...
                    for(i=16; i<16+8; i++){
                        if(h->non_zero_count_cache[ scan8[i] ] || h->mb[i*16]){
                            ...
                            ff_svq3_add_idct_c(ptr, h->mb + i*16, uvlinesize, ff_h264_chroma_qp[s->qp + 12] - 12, 2);
                        }
                    }
                }
            }
...
void ff_h264_hl_decode_mb(H264Context *h){
    ...
    const int mb_xy= h->mb_xy;
    const int mb_type= s->current_picture.mb_type[mb_xy];
    int is_complex = CONFIG_SMALL || h->is_complex || IS_INTRA_PCM(mb_type) || s->qp == 0;
...
//FIXME factorize this with the output code below
        ...
        if(out){
            av_log(avctx, AV_LOG_DEBUG, "out poc %d\n", out->poc);
