                   ...
                   int dir)
{
    ...
    if (s->ac_pred) {
        if (dir == 0) {
            ...
            if(s->mb_x==0 || s->qp == qscale_table[xy] || n==1 || n==3){
                /* same qscale */
                for(i=1;i<8;i++) {
                ...
                for(i=1;i<8;i++) {
                    block[s->dsp.idct_permutation[i<<3]] += ROUNDED_DIV(ac_val[i]*qscale_table[xy], s->qp);
                }
            }
        } else {
            ...
            if(s->mb_y==0 || s->qp == qscale_table[xy] || n==2 || n==3){
                /* same qscale */
                for(i=1;i<8;i++) {
                ...
                /* different qscale, we must rescale */
                for(i=1;i<8;i++) {
                    block[s->dsp.idct_permutation[i]] += ROUNDED_DIV(ac_val[i + 8]*qscale_table[xy], s->qp);
                }
            }
        }
    }
...
int mpeg4_decode_video_packet_header(MpegEncContext *s)
{
    ...
    if(s->shape != BIN_ONLY_SHAPE){
        int qp= get_bits(&s->gb, s->quant_precision);
        if(qp)
            s->chroma_qp=s->qp= qp;
    }
    ...
    for(; s->mb_y<s->mb_height; s->mb_y++){
        ...
        for(; s->mb_x<s->mb_width; s->mb_x++){
            ...
            if(s->pict_type==FF_I_TYPE){
                ...
                if(cbpc & 4) {
                    ff_set_qscale(s, s->qp + quant_tab[get_bits(&s->gb, 2)]);
                }
                s->current_picture.qscale_table[xy]= s->qp;
    ...
    if(intra) {
      ...
      qadd=0;
        ...
        if(s->mpeg_quant){
            ...
            qadd = (s->qp - 1) | 1;
            if(rvlc){
                rl_vlc = rvlc_rl_inter.rl_vlc[s->qp];
            }else{
                rl_vlc = ff_h263_rl_inter.rl_vlc[s->qp];
            }
        }
    }
  {
...
static int mpeg4_decode_partitioned_mb(MpegEncContext *s, DCTELEM block[6][64])
{
    ...
    s->use_intra_dc_vlc= s->qp < s->intra_dc_threshold;

    if(s->current_picture.qscale_table[xy] != s->qp){
    ...
    if (s->pict_type == FF_P_TYPE || s->pict_type==FF_S_TYPE) {
        ...
        if(modb1){
            ...
            if ((!IS_DIRECT(mb_type)) && cbp) {
                if(get_bits1(&s->gb)){
                    ff_set_qscale(s, s->qp + get_bits1(&s->gb)*4 - 2);
                }
            }
...
//FIXME complexity estimation stuff
     ...
     if (s->shape != BIN_ONLY_SHAPE) {
         s->chroma_qp= s->qp = get_bits(gb, s->quant_precision);
         if(s->qp==0){
