    ...
    for(n=0; n<6; n++){
        ...
        if(dir[n]){
            ...
            if(s->mb_y==0 || s->qp == qscale_table[xy] || n==2 || n==3){
                ...
                for(i=1; i<8; i++){
                    const int level= block[n][s->dsp.idct_permutation[i   ]];
                    block[n][s->dsp.idct_permutation[i   ]] = level - ROUNDED_DIV(ac_val[i + 8]*qscale_table[xy], s->qp);
                    ac_val1[i  ]=    block[n][s->dsp.idct_permutation[i<<3]];
                    ac_val1[i+8]= level;
                }
            }
            ...
            if(s->mb_x==0 || s->qp == qscale_table[xy] || n==1 || n==3){
                ...
                for(i=1; i<8; i++){
                    const int level= block[n][s->dsp.idct_permutation[i<<3]];
                    block[n][s->dsp.idct_permutation[i<<3]]= level - ROUNDED_DIV(ac_val[i]*qscale_table[xy], s->qp);
                    ac_val1[i  ]= level;
                    ac_val1[i+8]=    block[n][s->dsp.idct_permutation[i   ]];
                }
            }
            ...
            if ((cbp | motion_x | motion_y | s->dquant) == 0 && s->mv_type==MV_TYPE_16X16) {
                ...
                if(s->max_b_frames>0){
                    ...
                    for(i=0; i<s->max_b_frames; i++){
                        ...
                        if(diff>s->qp*70){ //FIXME check that 70 is optimal
                            s->mb_skipped=0;
                            break;
                        }
                    }
...
void ff_mpeg4_encode_video_packet_header(MpegEncContext *s)
{
    ...
    put_bits(&s->pb, s->quant_precision, s->qp);
