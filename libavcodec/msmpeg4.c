...
//           s->pict_type, best, s->qp, s->mb_var_sum, s->mc_mb_var_sum, best_size);
...
static void msmpeg4_encode_dc(MpegEncContext * s, int level, int n, int *dir_ptr)
{
    ...
    if(s->msmpeg4_version<=2){
        ...
        if(s->msmpeg4_version>=6 && s->qp<=2)
            extrabits = 3 - s->qp;

        if (code == DC_MAX)
            put_bits(&s->pb, 8 + extrabits, level);
        else if(extrabits > 0)//== VC1 && s->qp<=2
...
#if 0
{
...
#endif
    s->chroma_qp= s->qp = get_bits(&s->gb, 5);
    if(s->qp==0){
        av_log(s->avctx, AV_LOG_ERROR, "invalid qp\n");
        return -1;
    }
    ...
    if (s->pict_type == FF_I_TYPE) {
        ...
        if(s->avctx->debug&FF_DEBUG_PICT_INFO)
            av_log(s->avctx, AV_LOG_DEBUG, "qp:%d rlc:%d rl:%d dc:%d mbrl:%d slice:%d   \n",
                s->qp,
                              ...
                              int n, int coded, const uint8_t *scan_table)
{
    ...
    if (s->mb_intra) {
        ...
        if (level < 0){
            av_log(s->avctx, AV_LOG_ERROR, "dc overflow- block: %d qp: %d//\n", n, s->qp);
        ...
        if (n < 4) {
            ...
            if(level > 256*s->y_dc_scale){
                av_log(s->avctx, AV_LOG_ERROR, "dc overflow+ L qp: %d//\n", s->qp);
                if(!s->inter_intra_pred) return -1;
            }
        } else {
            rl = &rl_table[3 + s->rl_chroma_table_index];
            if(level > 256*s->c_dc_scale){
                av_log(s->avctx, AV_LOG_ERROR, "dc overflow+ C qp: %d//\n", s->qp);
                if(!s->inter_intra_pred) return -1;
            }
        }
        block[0] = level;
