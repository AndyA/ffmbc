...
static void channels_split(short **outputs, short *input, int n, int channels)
{
    int i, j;
...
static void channels_mux(short *output, short **inputs, int n, int channels)
{
    int i, j;
                                        ...
                                        int linear, double cutoff)
{
    ...
    if (input_channels > 8)
      {
        av_log(NULL, AV_LOG_ERROR, "Resampling with input channels greater than 8 unsupported.\n");
        return NULL;
      }

    if (input_channels > 2 && output_channels != input_channels) {
        av_log(NULL, AV_LOG_ERROR, "Changing channels with input channels greater than 2 unsupported.\n");
        return NULL;
    }
...
int audio_resample(ReSampleContext *s, short *output, short *input, int nb_samples)
{
    ...
    if (s->input_channels == 2 &&
        ...
        for (i = 0; i < s->output_channels; i++)
            buftmp3[i] = bufout[i];
        channels_split(buftmp2, input, nb_samples, s->input_channels);
    } else {
        buftmp3[0] = output;
        memcpy(buftmp2[0], input, nb_samples*sizeof(short));
    }
    ...
    if (s->output_channels == 2 && s->input_channels == 1) {
        ...
        channels_mux(output, buftmp3, nb_samples1, s->output_channels);
    }
