...
static int rv34_decode_macroblock(RV34DecContext *r, int8_t *intra_types)
{
    ...
    s->current_picture_ptr->qscale_table[mb_pos] = s->qp;

    if(cbp == -1)
        return -1;

    luma_dc_quant = r->block_type == RV34_MB_P_MIX16x16 ? r->luma_dc_quant_p[s->qp] : r->luma_dc_quant_i[s->qp];
    if(r->is16){
        memset(block16, 0, sizeof(block16));
        rv34_decode_block(block16, gb, r->cur_vlcs, 3, 0);
        rv34_dequant4x4_16x16(block16, rv34_qscale_tab[luma_dc_quant],rv34_qscale_tab[s->qp]);
    ...
    for(i = 0; i < 16; i++, cbp >>= 1){
        ...
        rv34_dequant4x4(s->block[blknum] + blkoff, rv34_qscale_tab[s->qp],rv34_qscale_tab[s->qp]);
    ...
    for(; i < 24; i++, cbp >>= 1){
        ...
        rv34_dequant4x4(s->block[blknum] + blkoff, rv34_qscale_tab[rv34_chroma_quant[1][s->qp]],rv34_qscale_tab[rv34_chroma_quant[0][s->qp]]);
        rv34_inv_transform(s->block[blknum] + blkoff);
    }
    if(IS_INTRA(s->current_picture_ptr->mb_type[mb_pos]))
