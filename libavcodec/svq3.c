...
static int svq3_decode_mb(SVQ3Context *svq3, unsigned int mb_type)
{
    ...
    if (IS_INTRA16x16(mb_type) || (s->pict_type != FF_I_TYPE && s->adaptive_quant && cbp)) {
        s->qp += svq3_get_se_golomb(&s->gb);

        if (s->qp > 31){
            av_log(h->s.avctx, AV_LOG_ERROR, "qp:%d\n", s->qp);
            return -1;
        }
    }
    ...
    if (cbp) {
        ...
        const int type = ((s->qp < 24 && IS_INTRA4x4(mb_type)) ? 2 : 1);
                             ...
                             AVPacket *avpkt)
{
    ...
    if (avctx->debug&FF_DEBUG_PICT_INFO){
        av_log(h->s.avctx, AV_LOG_DEBUG, "%c hpel:%d, tpel:%d aqp:%d qp:%d, slice_num:%02X\n",
               av_get_pict_type_char(s->pict_type), svq3->halfpel_flag, svq3->thirdpel_flag,
               s->adaptive_quant, s->qp, h->slice_num);
    }
