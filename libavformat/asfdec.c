...
static void get_strz16(ByteIOContext *pb, char *buf, int buf_size)
{
    char *q = buf;
    uint32_t ch;
    do {
        uint8_t tmp;
        GET_UTF16(ch, get_le16(pb), break;);
...
static int asf_read_cover(AVFormatContext *s, int type, int len)
{
    ...
    get_byte(s->pb); // type
    data_len = get_le32(s->pb);
    get_strz16(s->pb, mime, sizeof(mime));
    get_strz16(s->pb, NULL, 0);
    if (!(data = av_malloc(data_len)))
        return AVERROR_NOMEM;
    get_buffer(s->pb, data, data_len);
    if (av_metadata_set_custom(&s->metadata, &tag, METADATA_BYTEARRAY, "cover",
                               data, data_len, AV_METADATA_DONT_STRDUP_VAL) < 0)
        return -1;
...
static void get_tag(AVFormatContext *s, const char *key, int type, int len)
{
    ...
    if (type == 0) {         // UTF16-LE
        ...
        av_metadata_set_custom(&s->metadata, NULL, METADATA_BYTEARRAY,
                               key, value, len, AV_METADATA_DONT_STRDUP_VAL);
    } else if (type <= 5) {  // boolean or DWORD or QWORD or WORD
        uint64_t num = get_value(s->pb, type);
        av_metadata_set_int(&s->metadata, key, num);
    } else {
