...
static int asf_write_header1(AVFormatContext *s, int64_t file_size, int64_t data_chunk_size)
{
    ...
    tags[1] = av_metadata_get(s->metadata, "artist"   , NULL, 0);
    ...
    ff_metadata_conv2(&metadata, &s->metadata, ff_asf_metadata_conv, NULL);

    duration = asf->duration + PREROLL_TIME * 10000;
    has_title = tags[0] || tags[1] || tags[2] || tags[3] || tags[4];

    while ((tag = av_metadata_get(metadata, "", tag, 0)))
        if (!strncmp(tag->key, "WM/", 3))
            metadata_count++;
    ...
    if (metadata_count) {
        ...
        while ((tag = av_metadata_get(metadata, "", tag, AV_METADATA_IGNORE_SUFFIX))) {
            if (!strcmp(tag->key, "WM/Picture")) {
                ...
                put_byte(pb, 3);
                put_le32(pb, tag->len);
                avio_put_str16le(pb, mime);
                put_le16(pb, 0);
                put_buffer(pb, tag->value, tag->len);
