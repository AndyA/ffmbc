...
static int amf_parse_object(AVFormatContext *s, AVStream *astream, AVStream *vstream, const char *key, int64_t max_pos, int depth) {
    ...
    if(depth == 1 && key) { //only look for metadata values when we are not nested and key != NULL
        ...
        if(amf_type == AMF_DATA_TYPE_BOOL) {
            av_metadata_set_bool(&s->metadata, key, num_val);
        } else if(amf_type == AMF_DATA_TYPE_NUMBER) {
            av_metadata_set_float(&s->metadata, key, num_val);
...
static int flv_read_metabody(AVFormatContext *s, int64_t next_pos) {
    ...
    ByteIOContext *ioc;
    int i;
    char *buffer = NULL; //only needs to hold the string "onMetaData". Anything longer is something we don't want.
    ...
    type = get_byte(ioc);
    if(type != AMF_DATA_TYPE_STRING || amf_get_string(ioc, &buffer) < 0 || strcmp(buffer, "onMetaData")) {
        av_freep(&buffer);
        return -1;
    }
