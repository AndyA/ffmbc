                                 ...
                                 AVMetadata **metadata, char *prefix)
{
    ...
    for (i=0; i < list->nb_elem; i++) {
        AVMetadataTag *tag;
        if (av_metadata_set_custom(metadata, &tag, METADATA_STRING,
                                   tags[i].name, tags[i].string,
                                   strlen(tags[i].string),
                                   AV_METADATA_DONT_STRDUP_VAL |
                                   AV_METADATA_DONT_STRDUP_KEY) < 0)
            return;
        tags[i].name = NULL;
        tags[i].string = NULL;
...
static int matroska_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for (j=0; j<attachements_list->nb_elem; j++) {
        if (!(attachements[j].filename && attachements[j].mime &&
            ...
            if (av_metadata_set_custom(&s->metadata, &attachements[j].tag, METADATA_BYTEARRAY,
