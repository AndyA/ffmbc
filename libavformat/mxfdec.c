...
static int mxf_read_timecode_component(void *arg, ByteIOContext *pb, int tag, int size, UID uid)
{
    MXFTimecodeComponent *timecode = arg;
    switch(tag) {
    case 0x1501: // Start Time Code
        timecode->start = get_be64(pb);
        break;
    case 0x1502: // Rounded Time Code Base
        timecode->base = get_be16(pb);
        break;
    case 0x1503: // Drop Frame
        timecode->drop_frame = get_byte(pb);
        break;
    }
    return 0;
}
    ...
    for (i = 0; i < material_package->tracks_count; i++) {
        ...
        for (j = 0; j < material_track->sequence->structural_components_count; j++) {
            ...
            if (component->type == TimecodeComponent) {
                ...
                char timecode[32];
                ff_framenum_to_timecode(timecode, tc_component->start,
                                        tc_component->drop_frame, tc_component->base);
                av_metadata_set(&mxf->fc->metadata, "timecode", timecode);
                continue;
