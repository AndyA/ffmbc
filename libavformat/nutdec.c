...
static int get_str(ByteIOContext *bc, char **string){
    ...
    *string = NULL;
    ...
    if (len == UINT_MAX)
        return -1;
    *string = av_malloc(len+1);
    get_buffer(bc, *string, len);
    (*string)[len] = 0;
    return 0;
}
...
static int decode_info_header(NUTContext *nut){
    ...
    int chapter_id, i;
    int64_t value, end;
    char *str = NULL, *name, *type_str = NULL;
    const char *type;
