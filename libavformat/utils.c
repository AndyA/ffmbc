    ...
    for(;;){
        ...
        if (pktl) {
            ...
            if(   next_pkt->pts != AV_NOPTS_VALUE
                ...
                *pkt = *next_pkt;
                if (pkt->pts != AV_NOPTS_VALUE && pkt->pts > st->pts.val && pkt->duration > 0)
                    st->pts.val = pkt->pts + pkt->duration;
                if (genpts && eof && pkt->pts == AV_NOPTS_VALUE)
                    pkt->pts = st->pts.val;
                if(s->debug & FF_FDEBUG_TS)
...
static int has_codec_parameters(AVCodecContext *enc)
{
    ...
    case AVMEDIA_TYPE_DATA:
        return enc->codec_id != CODEC_ID_PROBE;
    default:
        val = 1;
        break;
    }
...
int av_write_header(AVFormatContext *s)
{
    ...
    /* set muxer identification string and time */
...
static int compute_pkt_fields2(AVFormatContext *s, AVStream *st, AVPacket *pkt){
    ...
    //av_log(s, AV_LOG_DEBUG, "av_write_frame: in  st:%d pts:%"PRId64" dts:%"PRId64""
    //       " duration:%d size:%d cur_dts:%"PRId64" b:%d\n",
    //       pkt->stream_index, pkt->pts, pkt->dts, pkt->duration, pkt->size,
    //       st->cur_dts, delay);
...
//        pkt->pts= st->cur_dts;
    ...
    //av_log(s, AV_LOG_DEBUG, "av_write_frame: out st:%d pts:%"PRId64" dts:%"PRId64" duration:%d b:%d\n",
    //       pkt->stream_index, pkt->pts, pkt->dts, pkt->duration, delay);
