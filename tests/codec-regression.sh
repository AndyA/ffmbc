...
if [ -n "$do_h261" ] ; then
do_video_encoding h261.avi "-qscale 11" "-vf scale=352:288 -an -vcodec h261"
do_video_decoding
fi

if [ -n "$do_h263" ] ; then
do_video_encoding h263.avi "-qscale 10" "-vf scale=352:288 -an -vcodec h263"
do_video_decoding
fi

if [ -n "$do_h263p" ] ; then
do_video_encoding h263p.avi "-qscale 2 -flags +umv+aiv+aic" "-vf scale=352:288 -an -vcodec h263p -ps 300"
do_video_decoding
fi
...
if [ -n "$do_snow" ] ; then
do_video_encoding snow.avi "-strict -2" "-an -vcodec snow -qscale 2 -flags +qpel -me_method iter -dia_size 2 -cmp 12 -subcmp 12 -vf scale=128:64,setsar=1:1"
do_video_decoding "" "-vf scale=352:288"
fi
...
if [ -n "$do_dv" ] ; then
do_video_encoding dv.dv "-dct int" "-vf scale=720:576 -an"
do_video_decoding "" "-vf scale=352:288"

do_video_encoding dv411.dv "-dct int" "-vf scale=720:576 -an -pix_fmt yuv411p -sws_flags area+accurate_rnd+bitexact"
do_video_decoding "" "-vf scale=352:288 -sws_flags area+accurate_rnd+bitexact"
fi

if [ -n "$do_dv50" ] ; then
do_video_encoding dv50.dv "-dct int" "-vf scale=720:576 -pix_fmt yuv422p -an -sws_flags neighbor+bitexact"
do_video_decoding "" "-vf scale=352:288 -pix_fmt yuv420p -sws_flags neighbor+bitexact"
fi

if [ -n "$do_dnxhd_1080i" ] ; then
do_video_encoding dnxhd-1080i.dnxhd "" "-flags +ildct -vf scale=1920:1080 -b 120M -pix_fmt yuv422p -vframes 5 -an"
do_video_decoding "-r 25" "-vf scale=352:288 -pix_fmt yuv420p"
fi

if [ -n "$do_dnxhd_720p" ] ; then
do_video_encoding dnxhd-720p.dnxhd "" "-vf scale=1280:720 -b 90M -pix_fmt yuv422p -vframes 5 -an"
do_video_decoding "-r 25" "-vf scale=352:288 -pix_fmt yuv420p"
fi

if [ -n "$do_dnxhd_720p_rd" ] ; then
do_video_encoding dnxhd-720p-rd.dnxhd "" "-threads 4 -mbd rd -vf scale=1280:720 -b 90M -pix_fmt yuv422p -vframes 5 -an"
do_video_decoding "-r 25" "-vf scale=352:288 -pix_fmt yuv420p"
fi
